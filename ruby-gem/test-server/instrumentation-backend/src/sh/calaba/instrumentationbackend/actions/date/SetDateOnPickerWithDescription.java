...
public class SetDateOnPickerWithDescription implements Action {
    ...
    public Result execute(String... args) {
        ...
        final Query query = new Query("android.widget.DatePicker {contentDescription LIKE[c] '" + description + "'");

        QueryResult queryResult = query.executeQuery();
        while (queryResult.isEmpty()) {
            if (System.currentTimeMillis() > startTime + timeoutInMillis) {
                return Result.failedResult("DatePicker with description " + description + " not found");
            }
